...
"##;
    ...
    /// Round-trip a plan whose project relation carries a (heterogeneous) struct
    /// literal constant expression (`{1_i32, "hello"}`) through text -> binary ->
    /// text, exercising both the struct-literal parser and printer.
    #[test]
    fn test_roundtrip_struct_literal() {
        use crate::proto::load_plan_from_binary;
